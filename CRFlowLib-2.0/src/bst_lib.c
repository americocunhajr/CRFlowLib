...
*  computational library with routines to manipulate a binary
*  search tree.
* -----------------------------------------------------------------
*/
...
void bst_leaf_free(void **leaf_bl)
{
    ...
    /* release allocated memory by bst_leaf */
    free(*leaf_bl);
    *leaf_bl = NULL;
...
void bst_node_free(void **node_bl)
{
    ...
    /* release allocated memory by bst_node elements */
    if( node->l_leaf != NULL )
    {
    ...
    /* release allocated memory by bst_node */
    free(*node_bl);
    *node_bl = NULL;
...
*   bst_cutplane
*
*   This function compute vector v = phi_r - phi_l
*   and scalar a = 0.5*(|phi_r|^2-|phi_l|^2)
*   that define a cutting plane used to search for
*   information in a binary search tree.
*   
*   Input:
...
int bst_node_add(unsigned int bst_side,
                 bst_node *old_node,
                 bst_node *new_node)
{
    if( bst_side == BST_RIGHT )
    {
